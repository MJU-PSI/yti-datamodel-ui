...
// import { flatten } from '@mju-psi/yti-common-ui';
...
export class InteractiveHelpPredicateService implements PredicateService, ResetableService {
  ...
  private store = new ModelResourceStore<Predicate>();
  ...
  getPredicate(id: Uri|Urn, model: Model): Promise<Predicate> {
    return Promise.resolve(this.store.getResourceForAnyModelById(id));
  }

  getAllPredicates(model: Model): Promise<Predicate[]> {
    return Promise.resolve(this.store.getResourceValuesForAllModels());
  }

  getRequiredByPredicates(model: Model): Promise<Predicate[]> {
    throw new Error('getRequiredByPredicates is not yet implemented in help');
  }

  getPredicatesForModel(model: Model): Promise<Predicate[]> {
    return this.store.getAllResourceValuesForModel(model);
  }
  ...
  getPredicatesAssignedToModel(model: Model): Promise<Predicate[]> {
    return this.store.getAllResourceValuesForModel(model);
  }
  ...
  newPredicate<T extends Attribute|Association>(model: Model, predicateLabel: string, conceptID: Uri|null, type: KnownPredicateType, lang: Language): Promise<T> {

    const conceptPromise: Promise<Concept|null> = conceptID ? this.helpVocabularyService.getConcept(conceptID) : Promise.resolve(null);
  ...
  newRelatedPredicate<T extends Attribute|Association>(model: Model, relatedClass: RelatedPredicate): Promise<T> {
    throw new Error('newRelatedPredicate is not yet implemented in help');
  }
  ...
  getExternalPredicate(externalId: Uri, model: Model): Promise<Predicate|null> {
    return Promise.resolve(this.store.findExternalResource(externalId));
  }

  getExternalPredicatesForModel(model: Model): Promise<Predicate[]> {
    const externalNamespaces = model.importedNamespaces.filter(ns => ns.external);
    const externalResources = flatten(externalNamespaces.map(ns => this.store.getExternalResourcesForNamespace(ns.url)));
    return Promise.resolve(externalResources);
  }
