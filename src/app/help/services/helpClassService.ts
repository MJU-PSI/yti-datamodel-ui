...
export class InteractiveHelpClassService implements ClassService, ResetableService {
  ...
  private store = new ModelResourceStore<Class>();
  ...
  getAllClasses(model: Model): Promise<Class[]> {
    return Promise.resolve(this.store.getResourceValuesForAllModels());
  }

  getRequiredByClasses(model: Model): Promise<Class[]> {
    throw new Error('getRequiredByClasses is not yet implemented in help');
  }

  getClassesForModel(model: Model): Promise<Class[]> {
    return this.store.getAllResourceValuesForModel(model);
  }
  ...
  getClassesAssignedToModel(model: Model): Promise<Class[]> {
    return this.store.getAllResourceValuesForModel(model);
  }
  ...
  newShape(classOrExternal: Class|ExternalEntity, profile: Model, external: boolean, lang: Language): Promise<Class> {
    ...
    const id = requireDefined(classOrExternal.id);
    const classPromise = (classOrExternal instanceof ExternalEntity) ? this.getExternalClass(id, profile).then(requireDefined) : Promise.resolve(classOrExternal);
  ...
  getExternalClass(externalId: Uri, _model: Model): Promise<Class|null> {
    return Promise.resolve(this.store.findExternalResource(externalId));
  }

  getExternalClassesForModel(model: Model): Promise<Class[]> {
    const externalNamespaces = model.importedNamespaces.filter(ns => ns.external);
    const externalResources = flatten(externalNamespaces.map(ns => this.store.getExternalResourcesForNamespace(ns.url)));
  ...
  getInternalOrExternalClass(id: Uri, _model: Model): Promise<Class|null> {
    return Promise.resolve(this.store.getInternalOrExternalResource(id));
  }
