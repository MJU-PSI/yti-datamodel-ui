// import { isDefined, Localizable, requireDefined } from '@mju-psi/yti-common-ui';
...
export class InteractiveHelpVocabularyService implements VocabularyService, ResetableService {
  ...
  getAllVocabularies(): Promise<Vocabulary[]> {
    return Promise.resolve(this.vocabularyStore.values());
  }

  searchConcepts(searchText: string, vocabulary?: Vocabulary): Promise<Concept[]> {
    return Promise.resolve(this.conceptStore.findAll(c => {
      const analysis = analyze(searchText, c, [x => x.label, x => x.definition]);
      return isDefined(analysis.matchScore) && (!vocabulary || vocabulary.id.equals(c.vocabulary.id));
    }));
  }

  createConceptSuggestion(vocabulary: Vocabulary, label: string, comment: string, lang: Language): Promise<Uri> {
  ...
  getConcept(id: Uri): Promise<Concept> {
    return Promise.resolve(requireDefined(this.conceptStore.findFirst(cs => cs.id.equals(id))));
  }

  createVocabulary(vocabulary: { prefix: string, label: Localizable, description: Localizable }): Promise<Vocabulary> {
    return this.entityCreatorService.createVocabulary(vocabulary)
      .then(v => this.vocabularyStore.add(v));
  }

  createConcept(vocabulary: Vocabulary, concept: { label: Localizable, definition: Localizable }): Promise<Concept> {
    return this.entityCreatorService.createConcept({
      vocabulary,
      index: ++this.conceptIndex,
      label: concept.label,
