import { containsAny, flatten, requireDefined, defaultLanguage } from '@mju-psi/yti-common-ui';
...
export interface PredicateService {
  getPredicate(id: Uri|Urn, model?: Model): Promise<Predicate>;
  getAllPredicates(model: Model): Promise<PredicateListItem[]>;
  getRequiredByPredicates(model: Model): Promise<PredicateListItem[]>;
  getPredicatesForModel(model: Model): Promise<PredicateListItem[]>;
  getPredicatesForModelDataSource(modelProvider: () => Model, requiredByInUse?: boolean): DataSource<PredicateListItem>;
  getPredicatesAssignedToModel(model: Model): Promise<PredicateListItem[]>;
  createPredicate(predicate: Predicate): Promise<any>;
  updatePredicate(predicate: Predicate, originalId: Uri, model: Model): Promise<any>;
  deletePredicate(id: Uri, model: Model): Promise<any>;
  assignPredicateToModel(predicateId: Uri, model: Model): Promise<any>;
  newPredicate<T extends Attribute | Association>(model: Model, predicateLabel: string, conceptID: Uri | null, type: KnownPredicateType, lang: Language | string): Promise<T>;
  newRelatedPredicate<T extends Attribute|Association>(model: Model, relatedPredicate: RelatedPredicate): Promise<T>;
  changePredicateType(predicate: Attribute|Association, newType: KnownPredicateType, model: Model): Promise<Attribute|Association>;
  copyPredicate(predicate: Predicate|Uri, type: KnownPredicateType, model: Model): Promise<Predicate>;
  getExternalPredicate(externalId: Uri, model: Model): Promise<Predicate|null>;
  getExternalPredicatesForModel(model: Model): Promise<PredicateListItem[]>;
...
//   private deserializePredicateList(data: GraphData): Promise<PredicateListItem[]> {
//     return this.frameService.frameAndMapArray(data, frames.predicateListFrame(data), () => PredicateListItem);
//   }

//   private deserializePredicate(data: GraphData, optional: boolean): Promise<Attribute|Association|Predicate|null> {
...
export class DefaultPredicateService implements PredicateService {
  ...
  getPredicate(id: Uri|Urn, model?: Model): Promise<Predicate> {
    return this.http.get<GraphData>(apiEndpointWithName('predicate'), {params: {id: id.toString()}})
      .toPromise()
      .then(response => this.deserializePredicate(response, false))
      .then(predicate => requireDefined(predicate));
  }
  ...
  getAllPredicates(model: Model): Promise<PredicateListItem[]> {
    return this.http.get<GraphData>(apiEndpointWithName('predicate'))
      .toPromise()
      .then(response => this.deserializePredicateList(response));
  }
  ...
  getPredicatesForModel(model: Model): Promise<PredicateListItem[]> {
    return this.getAllPredicates(model)
      .then(predicates => predicates.filter(predicate => predicate.id.resolves()));  // if resolves, it is known namespace
  }
  ...
  createPredicate(predicate: Predicate): Promise<any> {
    ...
    return this.http.put<{ identifier: Urn }>(apiEndpointWithName('predicate'), predicate.serialize(), {params: requestParams})
      ...
      .then(response => {
        this.modelPredicatesCache.delete(predicate.definedBy.id.uri);
        predicate.unsaved = false;
        predicate.version = response.identifier;
        predicate.createdAt = moment().utc();
        predicate.modifiedAt = moment().utc();
      });
  }
  ...
  getExternalPredicate(externalId: Uri, model: Model): Promise<Predicate|null> {
    return this.http.get<GraphData>(apiEndpointWithName('externalPredicate'), {params: {model: model.id.uri, id: externalId.uri}})
      .toPromise()
      .then(response => this.deserializePredicate(response, true))
      .then(predicate => {
        if (predicate) {
          predicate.external = true;
        }
