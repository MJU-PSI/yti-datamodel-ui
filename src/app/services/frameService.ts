...
export class FrameService {
  ...
  private frameData(data: GraphData, frame: any): Promise<GraphData> {
    if (!data || !frame) {
  ...
  frameAndMap<T extends GraphNode>(data: GraphData, optional: boolean, frame: {}, entityFactory: EntityFactory<T>): Promise<T|null> {
    return this.frameData(data, frame).then(framed => {
      try {
        if (framed['@graph'].length > 1) {
          throw new Error('Multiple graphs found: \n' + JSON.stringify(framed, null, 2));
        } else {
  ...
  frameAndMapArray<T extends GraphNode>(data: GraphData, frame: {}, entityFactory: EntityFactory<T>): Promise<T[]> {
    return this.frameData(data, frame).then(framed => {
      try {
        return normalizeAsArray(framed['@graph']).map(element => {
          const entity: EntityConstructor<T> = entityFactory(element);
  ...
  frameAndMapArrayEntity<T extends GraphNode, A extends GraphNodes<T>>(data: GraphData, frame: {}, entityArrayFactory: EntityArrayFactory<T, A>): Promise<A> {
