// import { index, normalizeAsArray, requireDefined } from '@mju-psi/yti-common-ui';
...
//   }
  ...
  // private deserializeModelPositions(data: GraphData): Promise<ModelPositions> {
...
export interface VisualizationService {
  getVisualization(model: Model): Promise<ClassVisualization>;
  updateModelPositions(model: Model, modelPositions: ModelPositions): Promise<any>;
}
...
export class DefaultVisualizationService implements VisualizationService {
  ...
  getVisualization(model: Model): Promise<ClassVisualization> {
    const classesPromise = this.getVisualizationClasses(model);
    const positionsPromise = this.getModelPositions(model);

    return Promise.all([classesPromise, positionsPromise])
      .then(([classes, positions]) => new ClassVisualization(classes, positions));
  }

  private getVisualizationClasses(model: Model): Promise<VisualizationClass[]> {
    const params = {
      graph: model.id.uri
    };
  ...
  private getModelPositions(model: Model): Promise<ModelPositions> {
    const params = { model: model.id.uri };

    return this.http.get<GraphData>(apiEndpointWithName('modelPositions'), { params })
      .toPromise()
      .then(expandContextWithKnownModels(model))
      .then(response => this.deserializeModelPositions(response), _err => this.newModelPositions(model))
      .catch(error => {
        console.log(error);
        throw error;
      });
  ...
  updateModelPositions(model: Model, modelPositions: ModelPositions): Promise<any> {
    ...
    return this.http.put(apiEndpointWithName('modelPositions'), modelPositions.serialize(), { params })
      .toPromise()
  }
