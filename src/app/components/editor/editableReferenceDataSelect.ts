...
// import { collectProperties, remove } from '@mju-psi/yti-common-ui';
...
@Component({
  ...
  templateUrl: './editableReferenceDataSelect.html'
})
export class EditableReferenceDataSelectComponent implements OnInit {
  ...
  ngOnInit() {
    this.descriptor = new ReferenceDataTableDescriptor(
      this.referenceData,
      this.model,
      this.languageService.createLocalizer(this.model),
      this.viewReferenceDataModal
    );
  }

  ngOnChanges(changes: SimpleChanges) {
    if(changes.referenceData && !changes.referenceData.firstChange) {
      this.descriptor = new ReferenceDataTableDescriptor(
        this.referenceData,
        this.model,
        this.languageService.createLocalizer(this.model),
  ...
  addReferenceData() {
    ...
    this.searchReferenceDataModal.openSelectionForProperty(this.model, exclude).then(
      (referenceData) => {
        this.expanded = true;
        this.referenceData.push(referenceData);
      },
