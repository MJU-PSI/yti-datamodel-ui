...
import { AddNew } from 'app/components/common/searchResults';
import { glyphIconClassForType } from 'app/utils/entity';
import { ChoosePredicateTypeModal } from './choose-predicate-type-modal';
...
export class SearchPredicateModal {
  ...
  openAddPredicate(model: Model, type: KnownPredicateType, exclude: Exclusion<AbstractPredicate> = noExclude): Promise<ExternalEntity | EntityCreation | Predicate> {
    return this.openModal(model, type, undefined, exclude, false, false);
  }

  openAddProperty(model: Model, klass: Class): Promise<Property> {
    const exclude = combineExclusions<PredicateListItem>(
      createExistsExclusion(collectProperties(klass.properties.filter(p => p.isAttribute()), p => p.predicateId.uri)),
      createDefinedByExclusion(model)
    );
    ...
    return this.openModal(model, null, undefined, exclude, false, true).then(predicate => {
      if (predicate instanceof Predicate && predicate.normalizedType === 'property') {
        return this.choosePredicateTypeModal.open().then((type: KnownPredicateType) => {
          return this.classService.newProperty(predicate, type, model);
        });
      } else {
        return this.classService.newProperty(predicate, predicate.normalizedType as KnownPredicateType, model);
  ...
  openWithCustomDataSource(
    model: Model,
    type: KnownPredicateType,
    dataSource: DataSource<PredicateListItem>,
  ...
  private openModal(
    ...
    customDataSource: DataSource<PredicateListItem> | undefined,
    exclude: Exclusion<AbstractPredicate>,
    onlySelection: boolean,
    allowExternal: boolean,
    requiredByInUse: boolean = false
  ): Promise<any> {
    const modalRef = this.modalService.open(SearchPredicateController, { size: 'lg' });
    const modalInstance = modalRef.componentInstance as SearchPredicateController;

    modalInstance.model = model;
    modalInstance.type = type;
    modalInstance.exclude = exclude;
    modalInstance.onlySelection = onlySelection;
    modalInstance.allowExternal = allowExternal;
    modalInstance.customDataSource = customDataSource;
    modalInstance.requiredByInUse = requiredByInUse;
...
export class SearchPredicateController implements SearchController<PredicateListItem> {
  ...
  searchResults: (PredicateListItem | AddNewPredicate)[] = [];
  selection: Predicate | ExternalEntity | any;
  selectionBefore: Predicate | ExternalEntity;
  searchText = '';
  typeSelectable: boolean;
  excludeError: string | null = null;
  cannotConfirm: string | null = null;
  ...
  search() {
    this.searchResults = [
      new AddNewPredicate(
        `${this.translateService.instant('Create new attribute')} '${this.searchText}'`,
        this.isAttributeAddable.bind(this),
        'attribute',
        false
      ),
      new AddNewPredicate(
        `${this.translateService.instant('Create new association')} '${this.searchText}'`,
        this.isAssociationAddable.bind(this),
        'association',
        false
      ),
      new AddNewPredicate(
        `${this.translateService.instant('Create new predicate')} ${this.translateService.instant('by referencing external uri')}`,
        () => this.canAddExternal(),
        this.type,
        true
      ),
  ...
  confirm() {
    ...
    if (selection instanceof Predicate) {
      if (selection.unsaved) {
        this.predicateService.createPredicate(selection)
          .then(() => this.activeModal.close(selection), err => this.excludeError = err.data.errorMessage);
      } else {
      ...
      if (this.externalPredicate) {
        ...
        if (exclude) {
          ...
          this.activeModal.close(this.externalPredicate);
        }
      } else {
        this.activeModal.close(selection);
      }
