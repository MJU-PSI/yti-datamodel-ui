...
export class SearchPredicateTableModal {
  ...
  private openModal(
    ...
    modalRef.componentInstance.predicatesAssignedToModel = predicatesAssignedToModel;
...
export class SearchPredicateTableComponent implements SearchController<PredicateListItem> {
  ...
  copyPredicate(item: AbstractPredicate) {
    this.activeModal.close(new RelatedPredicate(item.id, 'prov:wasDerivedFrom'));
  }

  createSubPredicate(item: AbstractPredicate) {
    this.activeModal.close(new RelatedPredicate(item.id, 'rdfs:subPropertyOf'));
  }

  createSuperPredicate(item: AbstractPredicate) {
    this.activeModal.close(new RelatedPredicate(item.id, 'iow:superPropertyOf'));
  }
