...
// import { Localizable, makeSimpleSearchRegexp } from '@mju-psi/yti-common-ui';
...
@Component({
  selector: 'highlight',
  template: '<span [innerHTML]="localizedText"></span>',
})
export class HighlightComponent implements OnInit {
  ...
  private formatText(): SafeHtml {
    ...
    if (regex.test(primaryTranslation)) {
      return this.sanitizer.bypassSecurityTrustHtml(applyHighlight(primaryTranslation, this.search));
    } else {
      const secondaryMatch = this.findSecondaryLanguageMatch(regex);

      if (secondaryMatch) {
        const formattedMatch = this.formatSecondaryLanguageMatch(secondaryMatch);
        const localizedText = `${primaryTranslation} [${formattedMatch}]`;
        return this.sanitizer.bypassSecurityTrustHtml(localizedText);
      } else {
        return this.sanitizer.bypassSecurityTrustHtml(primaryTranslation);
      }
    }
  }

  private findSecondaryLanguageMatch(regex: RegExp): { language: Language, startIndex: number, endIndex: number } | null {
    for (const [language, text] of Object.entries(this.text)) {
      const match = regex.exec(text);
      if (match) {
        const startIndex = match.index;
  ...
  private formatSecondaryLanguageMatch(match: { language: Language, startIndex: number, endIndex: number }): string {
    const secondaryTranslation = this.text[match.language];
    const excessiveThreshold = 30;
    const startIndex = Math.max(0, match.startIndex - excessiveThreshold);
    const endIndex = Math.min(secondaryTranslation.length, match.endIndex + excessiveThreshold);
...
export function HighlightFilter(sanitizer: DomSanitizer) {
  return (text: string, search: string) => {
    const highlightedText = applyHighlight(text, search);
    return sanitizer.bypassSecurityTrustHtml(highlightedText);
  };
}
