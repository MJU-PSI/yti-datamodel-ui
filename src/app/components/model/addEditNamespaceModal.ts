...
export class AddEditNamespaceModal {
  ...
  openAdd(context: LanguageContext, language: Language, reservedPrefixes?: string[], usedNamespaces: string[] = []): Promise<ImportedNamespace> {
    return this.open(context, language, null, this.concatPrefixes(reservedPrefixes), usedNamespaces);
  }

  openEdit(context: LanguageContext, require: ImportedNamespace, language: Language, reservedPrefixes?: string[], usedNamespaces: string[] = []): Promise<ImportedNamespace> {
    return this.open(context, language, require, this.concatPrefixes(reservedPrefixes), usedNamespaces);
  }
...
export class AddEditNamespaceModalComponent implements OnInit {
  ...
  create() {
    if (this.edit) {
      ...
      this.activeModal.close(this.mangleAsTechnicalIfNecessary(this.namespaceToEdit!));
    } else {
      this.modelService.newNamespaceImport(this.namespace, this.prefix, this.label, this.language)
        .then(ns => {
          return this.activeModal.close(this.mangleAsTechnicalIfNecessary(ns));
        }, err => this.submitError = err.data.errorMessage);
    }
  }
