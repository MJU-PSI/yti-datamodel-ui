...
// import { reversed } from '@mju-psi/yti-common-ui';
...
@Component({
  ...
  template: `
    <div (click)="setSortBy()">
      <button [id]="'sort_by_' + columnName + '_button'"
              type="button"
              class="btn btn-link pl-0"
              [ngClass]="{'sort-by-selected': isSortBySelected()}">
        {{headerText | translate}}
      </button>
      <i [id]="'sort_up_' + columnName + '_icon'" class="fas fa-sort-up primary-color-item" *ngIf="sortUp()"></i>
      <i [id]="'sort_down_' + columnName + '_icon'" class="fas fa-sort-down primary-color-item" *ngIf="sortDown()"></i>
    </div>
  `
})
export class SortByColumnHeaderComponent implements OnInit {
