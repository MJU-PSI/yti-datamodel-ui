...
// import { isDefined, arrowDown, arrowUp, enter, esc, pageDown, pageUp, tab, limit } from '@mju-psi/yti-common-ui';
...
@Component({
  ...
  template: `
    <div style="position: relative;">
      <ajax-loading-indicator-small *ngIf="pendingShow" class="autocomplete-pending-indicator"></ajax-loading-indicator-small>
    </div>
    <ng-content></ng-content>
    <!-- <input-popup [ctrl]="this"> -->
      <span class="content">{{format(match)}}</span>
    <!-- </input-popup> -->
  `
})
export class AutocompleteComponent<T> implements InputWithPopupController<T> {
  ...
  @Input() excludeProvider?: () => (item: T) => string;
  @Input() maxMatches: number;
  ...
  ngAfterViewInit() {
    ...
    const ngModel = inputElement?.ngModel;

    // this.autocompleteSubscription = ngModel?.valueChanges.pipe(
    //   tap(() => {
    //     // prevents initial triggering when user is not actually inputting anything
    //     if (document.activeElement === inputElement) {
    //       this.autocomplete(ngModel.value);
    //     }
    //   })
    // ).subscribe();

    const keyDownHandler = (event: KeyboardEvent) => this.keyPressed(event);
    // const focusHandler = () => {
    //   document.addEventListener('click', blurClickHandler);
    //   this.autocomplete(ngModel.value);
    // };
    // const blurClickHandler = (event: MouseEvent) => {
    //   const autocomplete = event.target.closest('autocomplete');

    //   if (autocomplete !== this.elementRef.nativeElement) {
    //     this.pendingShow = false;
    //     this.clear();
    //     document.removeEventListener('click', blurClickHandler);
    //   }
    // };
